...
- `src/main.rs` is a thin `tokio::main` wrapper: parse `Cli`, dispatch to `commands::{ask,auth,hot,quota,knowledge,pdf,ppt,user,search}::run`.
- `src/cli.rs` defines the clap-derived command tree, argument defaults, and value enums (`ModelTier`, `SearchDb`, `UserContentType`, `SortField`, `SortOrder`, `KbScope`, `RecallScope`). It also contains CLI-parsing unit tests.
...
- Knowledge-base and tools APIs share two daily quota pools (`knowledge`, `tools`); check them with `zhihu quota --ids knowledge,tools`. The quota query itself does not consume quota.
...
This repo includes a Codex skill under `skills/zhihu-cli/`. Install it with:
